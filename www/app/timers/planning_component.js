...
PlanningTimerSheet = function(options){
	...
	getPlanning = function () {
		...
		if(!defaults.vertical){
			...
			$thead.find('th[index]').each(function (index, col) {
				var day = { "day": index+1 , "triggers" :[]};

				var prev = null;
				var prevDeactivated = null;
				$tbody.find("td[index=" + (index +1) + "][class!='']" ).each(function (ndx, td) {
					var $this = $(td);
					var deactivated = $this.hasClass("deactivate");
					var cls = $this.prop("classList")[0];
					if (cls != prev || prevDeactivated != deactivated) {
	...
	showPlanning = function(){
		...
		$.each(planning, function (index, day) {
			var $tds = !defaults.vertical ? $tbody.find("tr[index="+ (day.day) + "] td") : $tbody.find("td[index="+ (day.day) + "]");
	...
	setModes = function(){
		...
		$.each(defaults.modes, function(index, mode){
			...
			_modes.push( "<div class='mode " + defaults.buttonClass +" " + ((index === modeSelected)?defaults.buttonModeClassSelected:"") +"' index=" + index +"><label title=" + modename + ">" + (!defaults.vertical ? modename : modename.substring(0,5)) + "</label>");
			if(defaults.temperatureModes)
	...
	setVertical = function($elt){
		if( window.matchMedia("(max-width: " + defaults.responsive_max_width +")").matches ){
			$elt.addClass("vertical");
